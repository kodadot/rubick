export const maxBuy = `SELECT 
  ...
  export const floorList = `SELECT date(e.timestamp), COALESCE(MIN(e.meta::bigint), 0) as value
    FROM event e 
    LEFT JOIN nft_entity ne on ne.id = e.nft_id 
  ...
  export const averageBuy = `SELECT date(e.timestamp), COALESCE(AVG(e.meta::bigint), 0) as value
    FROM event e 
    LEFT JOIN nft_entity ne on ne.id = e.nft_id 
    WHERE e.interaction = 'BUY' AND ne.collection_id = $1 
    GROUP BY date(e.timestamp) 
